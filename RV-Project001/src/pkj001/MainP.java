//Read TC and TS from Excel from different work sheet.
//right results of TC Execution in different sheet - Result, Error, Screenshot
...
public class MainP extends Utilities {
 	WebDriver myDriver;
...
//	ExtentTest testCase_Report,testSteps_Report;
	...
	static Logger logger = Logger.getLogger(MainP.class);
	...
	public void readTestData()	{   
		...
		TestRunName = "Demo_Costco"; 
		TempPath = "G:\\ITeLearn\\Excel Test Data and Results\\";
	
		fileInputPath = TempPath + "TestRun_" + TestRunName + ".xls";
		fileResultPath = TempPath + TestRunName + "_RunResult.xls";
...
//		Logger.info(fileInputPath);
	...
	@Test
	public void executeTestSteps() throws Exception  {
		
		Keywords lowLevelKeywords = new Keywords(myDriver);
	...
	//						testData=prevTSoutput;
						...
						*/
				    	...
				    	//	lowLevelKeywords.unHighlightElement(elementBy, elementID );
						    	...
						    	testCase_Report.appendChild(testSteps_Report);
						    	// break here from the outer loop if the Test Step is critical
						    	if (xlTestSteps[i][9].equalsIgnoreCase("Y")) {
						    		executeKW(lowLevelKeywords, "closeBrowser", elementBy, elementID, testData);
						    		testSteps_Report.log(LogStatus.ERROR,"Critical step failed");
						    		break testCaseExecution;
						    	}
					    }
//						System.out.println("in try");
	...
	public void executeKW(Keywords lowLevelKeywords, String strKeyWord, String ElementBy, String strElementID, String strTestData) throws Exception
	{
		String vReturn;
